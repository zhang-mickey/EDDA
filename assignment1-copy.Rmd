...
anova_model <- aov(Crops ~ County + Related + County:Related, data = df) 
summary(anova_model)
```

Since all p-values are large (≫ 0.05), we do not reject the null hypotheses, meaning there is no strong evidence that County or Related significantly impact Crops. <!--But we can see that the interaction is better.-->
...
We consider different ANCOVA models:
