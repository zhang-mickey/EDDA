...
**(a)**
Before we perform relevant ANOVA models,we need to check whether the data meets the necessary assumptions 
...
As there is no significance for the interaction model,we choose the pure model to estimate the data.
...
**(c)**
We can review the ANCOVA Model Results.
...
Significant County × Size (p \< 0.05) → This suggests that the effect of farm size on crop yield depends on the county.
The figure blow confirms this conclusion,as the slopes are different.
```{r}
library(ggplot2)
